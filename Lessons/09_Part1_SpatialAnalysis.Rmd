...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    mean_Ozone = mean(Ozone, na.rm=T), #What does "na.rm=T" do? ignore missing data and calculate mean
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(
    coords = c('mean_Lng','mean_Lat'), #longitude is x, latitude is y
    crs=4326
    )
...
mapview(EPAair.sf, cex="mean_PM25") #cex= changes size of the points
...
distance_matrix <- st_distance(EPAair.sf.utm, GasPlants.utm, pairwise=T) # 13 EPA sites, 32 gas plants
dim(distance_matrix)
