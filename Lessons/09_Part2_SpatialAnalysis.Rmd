...
st_bbox(counties_sf) #shows corners
...
ggplot(data=huc8_sf) + geom_sf(aes(color=ACRES))
...
ggplot()+
  ...
  geom_sf(data=un_sf) #check recording lol
...
# E_ is estimate, M_ is margin of error
...
ggplot() + 
  ...
  labs(fill='Socio\nEcononomic\nIndex')
#/n is like hitting enter for a label
...
wind_df <- read.csv(
  ...
  group_by(HUC_8) %>% 
  summarise(Wind = sum(AvgWindEnergy))
#substr(column we want it from, first, last)
  
#Join to HUC_8 features
huc8_wind <- huc8_sf %>% 
  left_join(wind_df)

#View the outputs with map
mapView(huc8_wind, zcol="Wind")
...
#google sf predicates to find more commands to be able to filter in other ways
