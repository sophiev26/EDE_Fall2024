...
* geom_line #adds line with intercept a and slope b
* geom_area
...
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point() #plots it as a point, not connected

O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot) #you can add multiple plots to one page if you print them after storing them in an object

# Fix this code
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) #specifying color in the aesthetics adds a layer to the plot instead of changing the color. must do it outside of the aesthehics command to do this
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #instead of coloring sites w/ different colors, we create one plot for each site
print(PMplot.faceted)

# Filter dataset within plot building and facet by multiple variables
PMplot.faceted2 <- #here, you can select only some sites by using the subset function
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #depth is in the reverse scale, so you want zero at the top and the highest number as the bottom
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #adds a trend line. lm = linear model
  geom_abline(aes(slope = 16, intercept = 0)) #adds another line with a slope and b intercept
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? bc color is the contour of the shape and contour is the filling of the shape if you change it to fill it will only make the current white part look like the color and the contour stays black which looks ugly
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) #binaxis: how the dots are stacking (over which axis), stackdir: how the dots are stacking (left aligned, centered, or right aligned etc), position tells you if they're overlaying or not
print(Nutrientplot4)
...
print(Nutrientplot6)
#no legend, hella problems. X axis only has label for first variable/nutrient
...
print(Nutrientplot7)
#this method for making frequency plots is better, that's why gather/spread is important!
