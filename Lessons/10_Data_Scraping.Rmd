...
check if a website has a robots.txt file. if they do, they don't want their data scraped
...
the_registrant <- webpage %>% 
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>% #tells R to look at elements on the webpage that meets those descriptions
  html_text() #this command gives the text associated with the node
the_registrant
```
...
df_withdrawals <- data.frame("Month" = rep(1:12), #creates sequence of numbers
                             "Year" = rep(2020,12), #12 instances of the year 2020
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #!! tells R this refers to a variable
...
scrape.it <- function(the_year, the_facility){
  ...
  Sys.sleep(5) #uncomment this if you are doing bulk scraping! Tells R to wait for 5 second or any number of seconds you add. This is done to keep the server from freaking out and thinking we're attacking the server. If this keeps happening from our IP address, Duke may lose access and it's just not nice! Someone doing an attack wouldn't wait 5+ seconds, so the server will realize that's not what's happening
...
the_dfs <- map(the_years,scrape.it,the_facility=my_facility) #does same thing as lapply 
...
dfs_2020 <- map2(the_years, the_facility_ids_subset, scrape.it) #iterates through these lists in parallel
