...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #opening the pipe
  mutate(month = month(sampledate)) %>% #mutate to create month column
  select(lakeid:daynum, month, sampledate:comments) %>% #select all columns to reorder
  drop_na(depth) #dropping all NAs
...
#saying all the nuterients from tn_ug to po4 should be combined into two columns, one for nutrient name and other with concentration
...
write.csv(NTL.nutrient.data.PeterPaul.gathered, row.names = FALSE, 
          file = "./Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaulGathered_Processed.csv") #stores the processed dataset
...
#opposite transformation from what we did with pivot_longer
...
LitterFiles
#combines all these common files

Litter <- LitterFiles %>%
  plyr::ldply(read.csv)
#this reads all the combined files
```
...
#Mixing the variables minues the common variables
...
NTL.PeterPaul.summaries <- 
  ...
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #filter NAs from these three columns. Ampersand means it'll give rows where none of these 3 have NAs
